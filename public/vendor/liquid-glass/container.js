  ...
  `
    ...
    void main() {
      ...
      vec2 center = vec2(0.5, 0.5);
        ...
        // Iridescent rim — prismatic edge refraction
        float rimEdge = rimIntensity * edgeIntensity;
        float irisAngle = atan(shapeNormal.y, shapeNormal.x);
        vec3 iris = vec3(
          0.70 + 0.30 * sin(irisAngle * 2.9 + 0.5),
          0.65 + 0.35 * sin(irisAngle * 2.9 + 2.3),
          1.00 + 0.25 * sin(irisAngle * 2.9 + 4.1)
        );
        vec3 irisWarm = vec3(1.0, 0.74, 0.42);
        float warmMix = exp(-normalizedDistance * 0.32) * 0.42;
        color.rgb = mix(color.rgb, color.rgb * iris + iris * 0.28, rimEdge * 0.92);
        color.rgb += irisWarm * warmMix * rimIntensity * 0.22;

        // Bright crescent highlight along the top
        float spec = exp(-coord.y * 7.0) * exp(-abs(coord.x - 0.5) * 3.5) * 0.32;
        color.rgb += vec3(spec);
    ...
    gl.uniform1f(blurRadiusLoc, window.glassControls?.blurRadius || 7.0)
    gl.uniform1f(borderRadiusLoc, this.borderRadius)
    gl.uniform1f(warpLoc, this.warp ? 1.0 : 0.0)
    gl.uniform1f(edgeIntensityLoc, window.glassControls?.edgeIntensity || 0.032)
    gl.uniform1f(rimIntensityLoc, window.glassControls?.rimIntensity || 0.12)
    gl.uniform1f(baseIntensityLoc, window.glassControls?.baseIntensity || 0.008)
    gl.uniform1f(edgeDistanceLoc, window.glassControls?.edgeDistance || 0.11)
    gl.uniform1f(rimDistanceLoc, window.glassControls?.rimDistance || 0.52)
    gl.uniform1f(baseDistanceLoc, window.glassControls?.baseDistance || 0.08)
    gl.uniform1f(cornerBoostLoc, window.glassControls?.cornerBoost || 0.055)
    gl.uniform1f(rippleEffectLoc, window.glassControls?.rippleEffect || 0.2)
